...
int main(int argc, char **argv)
{
  ...
  cudaMallocHost(&d_total_emitted_lava, sizeof(double));
  *d_total_emitted_lava = 0.0;

  double *d_total_current_lava = NULL;
  cudaMallocHost(&d_total_current_lava, sizeof(double));
  *d_total_current_lava = 0.0;
         ...
         (total_current_lava == -1 || total_current_lava > thickness_threshold))
  {
    ...
    // 1. EMIT LAVA
    *d_total_emitted_lava = sciara->simulation->total_emitted_lava; 
    ...
    if (sciara->simulation->step % reduceInterval == 0) {
      *d_total_current_lava = 0.0;
      ...
      cudaDeviceSynchronize();
      total_current_lava = *d_total_current_lava;
  ...
  cudaFreeHost(d_total_emitted_lava);
  cudaFreeHost(d_total_current_lava);
  finalize(sciara);
